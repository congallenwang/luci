...
------------------- Intel Device ------------------
if hwtype == "intel" then
	--country
	--local ctry = s:taboption("general", Value, "country", translate("Country Code"))
	--ctry.hwmodes = hw_modes
	--ctry.freqlist = iw.freqlist
	--ctry.template = "cbi/wireless_country"

	--Operation Mode
	op_mode = s:taboption("general", ListValue, "operation_mode", translate("Operation Mode"))
	if wdev:get("band") == "2.4G" then
		op_mode:value("b", translate("b only"))
		op_mode:value("bg", translate("bg"))
		op_mode:value("bgn", translate("bgn"))
	else
		op_mode:value("a", translate("a only"))
		op_mode:value("an", translate("an"))
		op_mode:value("anac", translate("anac"))
	end
	
--[[
	--Channel Bandwidth
	--20MHz, 40MHz, 80MHz, Auto
	local channel_width = s:taboption("general", Value, "channel_width", translate("Channel Bandwidth"))
	channel_width.hwmodes = hw_modes
	channel_width.freqlist = iw.freqlist
	channel_width.template = "cbi/wireless_channelwidth"

	--Secondary Channel
	--AboveControlChannel,BelowControlChannel,Auto
	local sec_channel = s:taboption("general", Value, "secondary_channel", translate("Secondary Channel"))
	--sec_channel:depends({channel="0"})
	--sec_channel:value("AboveControlChannel", translate("AboveControlChannel"))
	--sec_channel:value("BelowControlChannel", translate("BelowControlChannel"))
	--sec_channel:value("Auto", translate("Auto"))
	sec_channel.template = "cbi/wireless_sprefreq"
]]--	

--	ctry:value("auto")
--	ctry:value("CN", translate("bg"))
--	ctry:value("US", translate("bgn"))
--
	-----------------------------------------------------------------------------------------

	--s:taboption("advanced", Flag, "diversity", translate("Diversity")).rmempty = false
	local short_gi = s:taboption("advanced", ListValue, "short_gi", translate("Guard Interval"))
	short_gi:value("1", translate("400nsec"))
	short_gi:value("0", translate("800nsec"))
	--short,long,Auto --
	local preamble= s:taboption("advanced", ListValue, "preamble", translate("Preamble"))
	preamble:value("1", translate("short"))
	preamble:value("0", translate("long"))
	--Disabled,Enabled,RTS/CTS,CTS2Self
	local rts = s:taboption("advanced", ListValue, "s11nProtection", translate("Protection"))
	rts:value("0", translate("Disabled")) 
	rts:value("1", translate("Enabled"))
	rts:value("2", translate("RTS/CTS"))
	rts:value("3", translate("CTS2Self"))
	--Beamforming Mode
	--Auto,Explicit,Implicit,STBC1x2,STBC2x4,Disabled
	local bf_mode = s:taboption("advanced", ListValue, "bf_mode", translate("Beamforming Mode"))
	bf_mode:value("0xff", translate("Auto")) 
	bf_mode:value("0", translate("Explicit"))
	bf_mode:value("1", translate("Implicit"))
	bf_mode:value("2", translate("STBC1x2"))
	bf_mode:value("3", translate("STBC2x4"))
	bf_mode:value("4", translate("Disabled"))

	local ant = s:taboption("advanced", ListValue, "antenna", translate("Transmitter Antenna"))
	ant:value("1", translate("Antenna 1"))
	ant:value("2", translate("Antenna 2"))
	ant:value("3", translate("Antenna 3"))
	ant:value("4", translate("Antenna 4"))

	--Beacon Interval
	local beacon = s:taboption("advanced", Value, "beacon_period", translate("Beacon Interval"))
	beacon.datatype = "uinteger"
	--DTIM
	local dtim = s:taboption("advanced", Value, "dtim_period", translate("DTIM"))
	dtim.datatype = "uinteger"
	--Power scale
	--12,25,50,100
	local trans_power = s:taboption("advanced", ListValue, "transmit_power", translate("Power scale"))
	trans_power:value("12", translate("12"))
	trans_power:value("25", translate("25"))
	trans_power:value("50", translate("50"))
	trans_power:value("100", translate("100"))
	trans_power.default = "100"

	s:taboption("advanced", Flag, "Ht_STBC_enabled", translate("STBC"))
	s:taboption("advanced", Flag, "Ht_LDPC_enabled", translate("LDPC"))

	--CoexEnabled
	s:taboption("advanced", Flag, "coex_enable", translate("20/40 Mhz Co-Existence Enable"))
	--WaveIgnore40MhzIntolerant
	s:taboption("advanced", Flag, "ignore_40Mhz_intolerant", translate("Ignore 40MHz Intolerant"))
...
--	s:taboption("advanced", Flag, "outdoor", translate("Outdoor Channels"))
	...
	--WaveHtNumMsdusInAmsdu
	--2,3,4,5,6,7
	local ht_num_msdu = s:taboption("advanced", ListValue, "ht_num_msdu", translate("HT Number MSDUs in AMSDU"))
	ht_num_msdu:value("2", translate("2"))
	ht_num_msdu:value("3", translate("3"))
	ht_num_msdu:value("4", translate("4"))
	ht_num_msdu:value("5", translate("5"))
	ht_num_msdu:value("6", translate("6"))
	ht_num_msdu:value("7", translate("7"))

	--WaveHtNumMsdusInAmsdu
	--2,3,4,5,6,7
	local vht_num_msdu = s:taboption("advanced", ListValue, "vht_num_msdu", translate("VHT Number MSDUs in AMSDU"))
	vht_num_msdu:value("2", translate("2"))
	vht_num_msdu:value("3", translate("3"))
	vht_num_msdu:value("4", translate("4"))
	vht_num_msdu:value("5", translate("5"))
	vht_num_msdu:value("6", translate("6"))
	vht_num_msdu:value("7", translate("7"))

	--Max MPDU Length
	--WaveMaxMpduLen
	local max_mpdu_len = s:taboption("advanced", Value, "max_mpdu_len", translate("Max MPDU Length"))
	max_mpdu_len.datatype = "uinteger"
...
if hwtype == "mac80211" or hwtype == "prism2"  or hwtype=="intel" then
	local supplicant = fs.access("/usr/sbin/wpa_supplicant") 
	local hostapd = fs.access("/usr/sbin/hostapd") or fs.access("/opt/intel/bin/hostapd")

	-- Probe EAP support
	local has_ap_eap  = (os.execute("hostapd -veap >/dev/null 2>/dev/null") == 0) or hwtype=="intel" 
...
if hwtype == "mac80211" or hwtype == "prism2" or hwtype == "intel" then
	...
	local has_80211r = (os.execute("hostapd -v11r 2>/dev/null || hostapd -veap 2>/dev/null") == 0)  or  hwtype == "intel"
